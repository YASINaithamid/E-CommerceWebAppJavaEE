...
tableCommandesM=commandes.recuperercommandes_M(String.valueOf(session.getAttribute( "id_m" )),"commande envoyée"); 
commande commandee = new commande(0,"0","0","0","0",0,0,0,"0");
commandee.setcommande_id((commandes.cherchercommande((String) request.getParameter("id_Com"))).getcommande_id());
commandee.setdistinateur((commandes.cherchercommande((String) request.getParameter("id_Com"))).getdistinateur());
commandee.setdistinataire((commandes.cherchercommande((String) request.getParameter("id_Com"))).getdistinataire());
commandee.setdate_creation((commandes.cherchercommande((String) request.getParameter("id_Com"))).getdate_creation());
commandee.setid_produit((commandes.cherchercommande((String) request.getParameter("id_Com"))).getid_produit());
commandee.setid_c((commandes.cherchercommande((String) request.getParameter("id_Com"))).getid_c());
commandee.setQuantity((commandes.cherchercommande((String) request.getParameter("id_Com"))).getQuantity());
commandee.setsize((commandes.cherchercommande((String) request.getParameter("id_Com"))).getsize());
Products Products=new Products();
Product Productt=new Product(0,"0","0",0,0,"0","0");
String i=Integer.toString(commandee.getid_produit());
Productt.setproduit_id((Products.chercherProduct(i)).getproduit_id());
Productt.setname((Products.chercherProduct(i)).getname());
Productt.setdescription((Products.chercherProduct(i)).getdescription());
Productt.setid_M((Products.chercherProduct(i)).getid_M());
Productt.setprix((Products.chercherProduct(i)).getprix());
Productt.setsible((Products.chercherProduct(i)).getsible());
Productt.setimg((Products.chercherProduct(i)).getimg());
String j=Integer.toString(commandee.getid_c());
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                            <div class="card shadow mb-4">
                                ...
                                <form action="confirmerCommandeM?id_Com=<%=(String) request.getParameter("id_Com")%>" method="post">
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    <h6 class="m-0 font-weight-bold text-primary">Commande N° <%= request.getParameter("id_Com")%></h6>
                                ...
                                <div class="card-body">
                                    <div class="row pb-3 ">
				                        <div class="col-xl-4 col-lg-2 col-md-3 col-sm-3 col-12 pt-5 text-center">
				                          <img src=<%= Productt.getimg()%> height="200px">
				                        </div>
				                        <div class="col-xl-8 col-lg-10 col-md-8 col-sm-8 col-12 pt-5">
				                          <p>ID Commande :&nbsp;<%= request.getParameter("id_Com")%></p>
				                          <p>Date De Création :&nbsp;<%= commandee.getdate_creation()%></p>
				                          <p>Client :&nbsp;<%= Client.getNom() %> <%= Client.getPrenom() %></p>
				                          <p>Nom :&nbsp;<%= Productt.getname()%></p>
				                          <p>Prix par unité :&nbsp;<%= Productt.getprix()%> MAD</p>
				                          <p>Quantité :&nbsp;<%=commandee.getQuantity() %></p>
				                          <p>Taille :&nbsp;<%=commandee.getsize() %></p>
				                          <p>entrer la date de livraison :</p>
				                          <div class="form-group row">
  												<div class="col-10">
    												<input class="form-control" type="text" name="Date" value="2021-02-25" id="text">
  												</div>
										  </div>
				                        </div>
				                      </div>
